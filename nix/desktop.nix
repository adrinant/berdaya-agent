# nix/desktop.nix — Berdaya Agent Desktop (Electron) app build + wrapper
#
...
# uses our fully wrapped binary at step 4 ("existing Berdaya Agent CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # binary so the desktop's resolver step 4 ("existing Berdaya Agent CLI on
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Berdaya Agent";
