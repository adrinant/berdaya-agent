...
Owns one Codex thread per Berdaya Agent session. Drives `turn/start`, consumes
...
# Berdaya Agent' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Berdaya Agent/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Berdaya Agent session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        into Berdaya Agent' messages shape.
        ...
        # supports rich content but Berdaya Agent' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Berdaya Agent' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Berdaya Agent' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Berdaya Agent approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Berdaya Agent returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Berdaya Agent version string for codex's userAgent line."""
    try:
