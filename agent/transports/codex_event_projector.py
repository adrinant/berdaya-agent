"""Projects codex app-server events into Berdaya Agent' messages list.

The translator that lets Berdaya Agent' memory/skill review keep working under the
...
Berdaya Agent' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Berdaya Agent
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Berdaya Agent' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Berdaya Agent stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Berdaya Agent already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Berdaya Agent' messages store text only.
        text_parts: list[str] = []
