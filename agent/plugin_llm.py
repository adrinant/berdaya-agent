...
Plugins built on Berdaya Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Berdaya Agent
...
embedded slugs, no shorthands. This mirrors Berdaya Agent' main config
...
handles every provider, fallback chain, and per-task override Berdaya Agent
supports.
"""
