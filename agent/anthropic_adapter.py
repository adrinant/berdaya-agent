"""Anthropic Messages API adapter for Berdaya Agent.

Translates between Berdaya Agent's internal OpenAI-style message format and
...
# Berdaya Agent effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Berdaya Agent historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Berdaya Agent)
    ...
    # 1. Berdaya-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Berdaya Agent configs.
...
# ── Berdaya Agent-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Berdaya Agent-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Berdaya Agent with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Berdaya-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Berdaya Agent OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Berdaya Agent", "Claude Code")
                text = text.replace("Berdaya Agent agent", "Claude Code")
    ...
    # silently hides reasoning text that Berdaya Agent surfaces in its CLI. We
