---
sidebar_position: 8
title: "在 Berdaya Agent 中使用语音模式"
description: "在 CLI、Telegram、Discord 及 Discord 语音频道中设置和使用 Berdaya Agent 语音模式的实用指南"
---

# 在 Berdaya Agent 中使用语音模式
...
- 希望 Berdaya Agent 加入 Discord 语音频道进行实时对话
- 边走动边快速记录想法、调试问题或来回交流，而不是打字

## 选择你的语音模式方案

Berdaya Agent 中实际上有三种不同的语音体验。
...
如果你在设置向导中选择了 NeuTTS，Berdaya Agent 会检查 `neutts` 是否已安装。如果缺失，向导会告知你 NeuTTS 需要 Python 包 `neutts` 和系统包 `espeak-ng`，并提供自动安装，使用平台包管理器安装 `espeak-ng`，然后运行：
...
- 边走动边思考
- 口述半成形的想法
- 让 Berdaya Agent 实时整理你的思路

#### 无障碍 / 少打字场景

如果打字不方便，语音模式是保持完整 Berdaya Agent 工作流的最快方式之一。

## 调整 CLI 行为

### 静音阈值

如果 Berdaya Agent 开始/停止过于激进，调整：
...
Berdaya Agent 仍作为普通聊天机器人运行，但可以朗读回复。
...
Berdaya Agent 加入 Discord 语音频道（VC），监听用户语音，转录后运行正常的 agent 流水线，并将回复朗读回频道。
...
- 用户在语音频道中说话
- Berdaya Agent 检测语音边界
- 转录内容发布到关联的文本频道
- Berdaya Agent 以文字和音频形式回复
