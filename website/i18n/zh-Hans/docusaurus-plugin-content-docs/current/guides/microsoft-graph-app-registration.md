...
4. 填写以下内容：
   - **Name：**`Berdaya Agent Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Berdaya Agent app
New-CsApplicationAccessPolicy `
  -Identity "Berdaya Agent-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Berdaya Agent meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Berdaya Agent-Meeting-Pipeline-Policy" `
...
Berdaya Agent 内置了 Graph 身份验证冒烟测试。在 Berdaya Agent 安装目录下执行：
