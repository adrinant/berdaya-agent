...
Curator 由空闲检查触发，而非 cron 守护进程。在 CLI 会话启动时，以及 gateway 的 cron-ticker 线程内的周期性 tick 中，Berdaya Agent 会检查以下条件是否同时满足：
...
在每次真正的 curator pass 之前，Berdaya Agent 会在 `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz` 处对 `~/.hermes/skills/` 进行 tar.gz 快照。如果某次 pass 归档或合并了你不希望被触碰的内容，可以用一条命令撤销整次运行：
...
回滚本身也是可逆的：在替换技能树之前，Berdaya Agent 会再次创建一个标记为 `pre-rollback to <target-id>` 的快照，因此误操作的回滚可以通过 `--id` 滚动到该快照来撤销。
...
- 通过你指向 Berdaya Agent 的外部技能目录添加的技能。
