...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Berdaya Agent"
---

# ACP 编辑器集成

Berdaya Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Berdaya Agent 通信并渲染：
...
当你希望 Berdaya Agent 表现得像编辑器原生的编码 agent，而非独立 CLI 或消息机器人时，ACP 是合适的选择。

## Berdaya Agent 在 ACP 模式下暴露的内容

Berdaya Agent 使用专为编辑器工作流设计的精选 `hermes-acp` 工具集运行，包括：
...
正常安装 Berdaya Agent 后，添加 ACP 扩展：
...
对于 Zed registry 安装，Zed 通过官方 ACP Registry 条目启动 Berdaya Agent。该条目使用 `uvx` 发行版运行：
...
以下任意命令均可以 ACP 模式启动 Berdaya Agent：
...
Berdaya Agent 将日志输出到 stderr，以保留 stdout 用于 ACP JSON-RPC 流量。
...
- 将 `npm install -g agent-browser @askjo/camofox-browser` 安装到该前缀（无需 sudo — `npm` 的 `--prefix` 指向用户可写的 Berdaya Agent 管理 Node）
...
2. 从内置 agent 列表中选择 **Berdaya Agent**。
3. 连接并开始聊天。

如需手动定义 Berdaya Agent，通过 VS Code 设置在 `acp.agents` 下添加：
...
3. 搜索 **Berdaya Agent**。
4. 安装后启动新的 Berdaya Agent 外部 agent 线程。

前提条件：

- 先通过 `hermes model` 配置 Berdaya Agent provider 凭据，或在 `~/.hermes/.env` / `~/.hermes/config.yaml` 中设置。
...
Berdaya Agent 官方 ACP Registry 元数据的源文件位于：
...
ACP 模式使用与 CLI 相同的 Berdaya Agent 配置：
...
Provider 解析使用 Berdaya Agent 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。Berdaya Agent 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 Berdaya Agent 的交互式模型/provider 设置。
...
底层 `AIAgent` 仍使用 Berdaya Agent 的正常持久化/日志路径，但 ACP 的 `list/load/resume/fork` 仅限于当前运行的 ACP 服务器进程。

## 工作目录行为

ACP 会话将编辑器的 cwd 绑定到 Berdaya Agent 任务 ID，使文件和终端工具相对于编辑器工作区运行，而非服务器进程的 cwd。
...
| `allow_always` | 始终允许 | 所有未来会话 | 是（写入 Berdaya Agent 永久允许列表） |
| `deny` | 拒绝 | 本次工具调用 | 否 |
...
ACP 桥接将这些选项映射到 Berdaya Agent 的内部审批语义——`allow_always` 与 CLI 相同地写入永久允许列表条目，而 `allow_session` 仅影响当前 ACP 会话的进程内审批缓存。
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **Berdaya Agent**。
- 对于手动/本地开发，验证自定义 `agent_servers` 命令是否指向 `hermes acp`。
- Berdaya Agent 已安装且在 PATH 中。
...
ACP 模式使用 Berdaya Agent 现有的 provider 设置。通过以下方式配置凭据：

```bash
hermes model
```

或编辑 `~/.hermes/.env`。Registry 客户端也可以触发 Berdaya Agent 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Berdaya Agent 线程。
