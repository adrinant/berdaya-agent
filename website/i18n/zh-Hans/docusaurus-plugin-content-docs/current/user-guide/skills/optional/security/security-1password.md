...
| 版本 | `1.0.0` |
| 作者 | arceus77-7，由 Berdaya Agent 增强 |
| 许可证 | MIT |
...
- `tmux` 可用，用于在 Berdaya Agent 终端调用期间保持稳定的已认证会话（仅限桌面应用流程）
...
## Berdaya Agent 执行模式（桌面应用流程）

Berdaya Agent 终端命令默认为非交互式，且在多次调用之间可能丢失认证上下文。
