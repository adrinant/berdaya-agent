...
sidebar_position: 1
title: "架构"
description: "Berdaya Agent 内部结构——主要子系统、执行路径、数据流及延伸阅读指引"
---

# 架构

本页是 Berdaya Agent 内部结构的顶层导图。用它在代码库中定位自己，然后深入各子系统专项文档了解实现细节。
...
├── cli.py                    # Berdaya AgentCLI — 交互式终端 UI（大文件）
...
```text
用户输入 → Berdaya AgentCLI.process_input()
...
通过 stdio/JSON-RPC 将 Berdaya Agent 作为编辑器原生 agent 暴露给 VS Code、Zed 和 JetBrains。
