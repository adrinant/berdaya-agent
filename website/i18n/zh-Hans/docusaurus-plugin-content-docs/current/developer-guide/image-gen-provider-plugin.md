...
description: "如何为 Berdaya Agent 构建图像生成后端插件"
---
...
:::tip
图像生成是 Berdaya Agent 支持的多种**后端插件**之一。其他插件（各有更专用的 ABC）包括：[Memory Provider 插件](/developer-guide/memory-provider-plugin)、[Context Engine 插件](/developer-guide/context-engine-plugin) 和 [Model Provider 插件](/developer-guide/model-provider-plugin)。通用工具/hook/CLI 插件请参阅 [构建 Berdaya Agent 插件](/guides/build-a-hermes-plugin)。
:::

## 发现机制

Berdaya Agent 在三个位置扫描图像生成后端：
