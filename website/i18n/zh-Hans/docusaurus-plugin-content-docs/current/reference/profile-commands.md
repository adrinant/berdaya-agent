...
本页涵盖所有与 [Berdaya Agent profiles](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
此处显示的是 profile 的 Berdaya Agent 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 启动（或在本地后端 `cwd: "."` 时从启动目录启动）。
...
重新生成位于 `~/.local/bin/<name>` 的 shell alias 脚本。适用于 alias 被意外删除，或移动 Berdaya Agent 安装目录后需要更新的情况。
...
打印 profile 的发行版 manifest — 名称、版本、所需 Berdaya Agent 版本、作者、环境变量要求、来源 URL/路径，以及发行版最后一次 `install` 或 `update` 时记录的 `Installed:` 时间戳。适用于安装前检查共享 profile 的需求，以及发现"该 profile 已安装 6 个月未更新"等情况。
...
`hermes_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 Berdaya Agent 版本不满足规格，安装将失败并给出明确错误。
...
2. 初始化 git 仓库（或使用已有仓库），推送到 GitHub / GitLab / 任何 Berdaya Agent 可克隆的托管平台。
...
全局标志，用于在不更改默认 profile 的情况下，在指定 profile 下运行任意 Berdaya Agent 命令。仅在该命令执行期间覆盖活跃 profile。
