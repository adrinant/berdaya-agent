...
This page covers all commands related to [Berdaya Agent profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
This shows the profile's Berdaya Agent home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Berdaya Agent installation.
...
Berdaya Agent version, author, env var requirements, the source URL/path, and
...
Berdaya Agent version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Berdaya Agent can clone from.
...
Global flag to run any Berdaya Agent command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
