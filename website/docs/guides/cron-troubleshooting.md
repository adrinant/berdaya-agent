...
description: "Diagnose and fix common Berdaya Agent cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the Berdaya Agent config directory). Verify:
```bash
...
hermes logs                         # View recent Berdaya Agent logs
