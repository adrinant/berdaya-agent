---
sidebar_position: 6
title: "Use MCP with Berdaya Agent"
description: "A practical guide to connecting MCP servers to Berdaya Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Berdaya Agent

This guide shows how to actually use MCP with Berdaya Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Berdaya Agent tool
- you want Berdaya Agent to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Berdaya Agent to internal APIs, databases, or company systems without modifying Berdaya Agent core

Do not use MCP when:
- a built-in Berdaya Agent tool already solves the job well
...
- Berdaya Agent remains the agent
- MCP servers contribute tools
- Berdaya Agent discovers those tools at startup or reload time
...
If you installed Berdaya Agent with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Berdaya Agent banner/status should show MCP integration when configured
- ask Berdaya Agent what tools it has available
...
## WSL2: bridge Berdaya Agent in WSL to Windows Chrome

This is the practical setup when:

- Berdaya Agent runs inside WSL2
...
In this setup, Berdaya Agent does **not** connect to Chrome directly. Instead:

- Berdaya Agent runs in WSL
- Berdaya Agent starts a local stdio MCP server
...
```text
Berdaya Agent (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Berdaya Agent stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Berdaya Agent core browser transport
...
Then start a fresh Berdaya Agent session or run:
...
Once loaded, Berdaya Agent can use the MCP-prefixed browser tools directly. For example:
...
If Berdaya Agent runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Berdaya Agent from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Berdaya Agent from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Berdaya Agent:
...
2. Berdaya-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Berdaya Agent will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Berdaya Agent to reason over a bounded workspace.
...
Now Berdaya Agent can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Berdaya Agent core.
...
Because Berdaya Agent now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
