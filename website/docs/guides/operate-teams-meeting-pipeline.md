...
#### Option 1: Berdaya Agent cron (recommended if you already run the Berdaya Agent gateway)

Berdaya Agent ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
```ini
[Unit]
Description=Berdaya Agent Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run Berdaya Agent Teams pipeline subscription maintenance every 12 hours
...
- [ ] **`maintain-subscriptions` is scheduled** (Berdaya Agent cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
