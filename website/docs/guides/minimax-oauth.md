...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Berdaya Agent — no API key required"
---

# MiniMax OAuth

Berdaya Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Berdaya Agent automatically refreshes your session.
...
# → Berdaya Agent opens your browser to the MiniMax authorization page
...
Berdaya Agent will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Berdaya Agent implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Berdaya Agent generates a PKCE verifier / challenge pair and a random state value.
...
4. Berdaya Agent polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
...
Berdaya Agent refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Berdaya Agent marks the session as requiring re-login.
...
The device-code flow has a finite expiry window. If you don't approve the login in time, Berdaya Agent raises a timeout error.

**Fix:** re-run `hermes auth add minimax-oauth` (or `hermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

Berdaya Agent detected that the `state` value returned by the authorization server does not match what it sent.
...
Berdaya Agent prints the URL and code. Open the URL on any device and complete the flow there.
