...
messaging platform Berdaya Agent is already configured for. Think of it as a
...
Any platform Berdaya Agent ships adapters for works as a target:
...
When stdin is a TTY (no pipe), Berdaya Agent does **not** wait for input — you'll
...
Because `hermes send` reuses your Berdaya Agent config, the same script works on
any host where Berdaya Agent is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses Berdaya Agent creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
