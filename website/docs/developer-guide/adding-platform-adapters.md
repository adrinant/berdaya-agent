...
This guide covers adding a new messaging platform to the Berdaya Agent gateway. A platform adapter connects Berdaya Agent to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Berdaya Agent code. Your plugin is a directory with two files:
...
version: 1.0.0
description: >
  My Platform gateway adapter for Berdaya Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Berdaya Agent core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
