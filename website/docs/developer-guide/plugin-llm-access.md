...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Berdaya Agent already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Berdaya Agent provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Berdaya Agent provider
        # (must be one Berdaya Agent already knows about — same names as
        ...
        # plugin sends — Berdaya Agent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Berdaya Agent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Berdaya Agent subsystem:
