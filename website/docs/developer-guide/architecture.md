...
sidebar_position: 1
title: "Architecture"
description: "Berdaya Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Berdaya Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # Berdaya AgentCLI — interactive terminal UI (large file)
...
```text
User input → Berdaya AgentCLI.process_input()
...
Exposes Berdaya Agent as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
