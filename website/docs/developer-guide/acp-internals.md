...
The ACP adapter wraps Berdaya Agent' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
hermes acp / hermes-acp / python -m acp_adapter
  ...
  -> construct Berdaya AgentACPAgent
...
- `allow_once` -> Berdaya Agent `once`
- `allow_always` -> Berdaya Agent `always`
- reject options -> Berdaya Agent `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Berdaya Agent tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Berdaya Agent' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Berdaya Agent provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Berdaya Agent' interactive model/provider configuration before starting a normal ACP session.
