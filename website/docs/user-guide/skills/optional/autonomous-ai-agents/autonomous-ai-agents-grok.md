...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Berdaya Agent |
| License | MIT |
...
# Grok Build CLI — Berdaya Agent Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Berdaya Agent terminal. Grok
...
1. Prepare stable input files first with Berdaya Agent tools (`read_file`,
...
2. **Don't conflate Berdaya Agent' xAI auth with the `grok` CLI's auth.** Berdaya Agent'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Berdaya Agent' xAI auth carries
   over.
