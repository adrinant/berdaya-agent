...
| Version | `0.1.0` |
| Author | Tony Simons (asimons81), Berdaya Agent |
| License | MIT |
...
commands through the Berdaya Agent `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Berdaya Agent itself.
