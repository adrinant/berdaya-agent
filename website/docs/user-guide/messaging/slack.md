...
sidebar_position: 4
title: "Slack"
description: "Set up Berdaya Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Berdaya Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Berdaya Agent instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Berdaya Agent uses the modern
...
The fastest path is to paste a manifest Berdaya Agent generates for you. It
...
### Option A: From a Berdaya-generated manifest (recommended)
...
4. Enter an app name (e.g., "Berdaya Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Berdaya Agent can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Berdaya Agent configuration issue.
:::
...
Berdaya Agent uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every Berdaya Agent command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Berdaya Agent command with its description.

Under the hood: Berdaya Agent ships with a generated Slack app manifest (see
...
When Berdaya Agent adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — Berdaya Agent routes it the same way as `/btw
...
Slack never delivers them to Berdaya Agent.

As a workaround, Berdaya Agent recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Berdaya Agent treats it identically to the slash form and replies in the same
thread.
...
Berdaya Agent falls back to a text prompt, the prompt instructs you to reply
...
Understanding how Berdaya Agent behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Berdaya Agent what time is it?`). In channels, Berdaya Agent replies in a thread attached to that message. |
| **Threads** | If you @mention Berdaya Agent inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Berdaya Agent in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Berdaya Agent will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Berdaya Agent will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Berdaya Agent`).

---

## Multi-Workspace Support

Berdaya Agent can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Berdaya Agent also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Berdaya Agent uses the correct workspace-specific client to respond.
...
Berdaya Agent supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Berdaya Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Berdaya Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Berdaya Agent now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Berdaya Agent`)
...
- Audit who has access to your Berdaya Agent config directory
