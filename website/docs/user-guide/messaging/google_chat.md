...
sidebar_position: 12
title: "Google Chat"
description: "Set up Berdaya Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Berdaya Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Berdaya Agent
...
download the file. Save it somewhere only Berdaya Agent can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Berdaya Agent calls
...
- **App name**: whatever you want users to see ("Berdaya Agent" is reasonable).
...
`ADDED_TO_SPACE` event that Berdaya Agent uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Berdaya Agent
...
Install the dependencies the Google Chat adapter needs (no Berdaya Agent extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Berdaya Agent is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Berdaya Agent detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Berdaya Agent session.
...
3. Download the JSON. Move it onto the host that runs Berdaya Agent.
4. Register the client with Berdaya Agent (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Berdaya Agent home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Berdaya Agent isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Berdaya Agent will only attach the SA bearer
