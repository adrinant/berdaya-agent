...
Connect Berdaya Agent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Berdaya Agent supports two WeCom integration modes:
...
3. Berdaya Agent decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in Berdaya Agent and the token the
