...
title: "SMS (Twilio)"
description: "Set up Berdaya Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Berdaya Agent connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
## Step 2: Configure Berdaya Agent
...
If you're running Berdaya Agent locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Berdaya Agent will respond via SMS.
...
Berdaya Agent validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Berdaya Agent gateway logs for Twilio API errors
