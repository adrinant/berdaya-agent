...
Connect Berdaya Agent to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Berdaya Agent — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Berdaya Agent runs it in a
...
2. **Finds or creates** the `Berdaya Agent` project on your account.
...
Photon uses the same authorization model as every other Berdaya Agent
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Berdaya Agent replies with a pairing code. Approve it with:
...
By default Berdaya Agent responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Berdaya Agent` and
`@Berdaya Agent agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Berdaya Agent will reply.
...
assigned iMessage line Berdaya Agent uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Berdaya Agent sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Berdaya Agent wake words  | JSON list / comma / newline regex patterns for group mentions |
