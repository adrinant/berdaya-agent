...
2. Berdaya Agent stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, Berdaya Agent calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Berdaya Agent **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Berdaya Agent process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "Berdaya Agent keys").
...
4. **Machine accounts → New machine account → My Berdaya Agent machine** → **Projects** tab → grant Read access to your project.
...
Bitwarden never blocks Berdaya Agent startup. If anything goes wrong, you'll see a one-line warning in stderr and Berdaya Agent continues with whatever credentials `.env` already had:
...
- Berdaya Agent will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Berdaya Agent does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Berdaya Agent installations.
