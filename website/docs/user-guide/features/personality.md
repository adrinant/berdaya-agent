...
description: "Customize Berdaya Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Berdaya Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Berdaya Agent is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Berdaya Agent now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Berdaya Agent with a custom home directory, it will use:
...
- Berdaya Agent creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Berdaya Agent loads `SOUL.md` only from `HERMES_HOME`
- Berdaya Agent does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Berdaya Agent falls back to a built-in default identity
...
If Berdaya Agent loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Berdaya Agent instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change Berdaya Agent' default personality."
...
- how Berdaya Agent should handle uncertainty, disagreement, or ambiguity
...
## What Berdaya Agent injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Berdaya Agent falls back to a built-in default identity ("You are Berdaya Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Berdaya Agent ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Berdaya Agent, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Berdaya Agent its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Berdaya Agent speaks
- `display.skin` and `/skin` affect how Berdaya Agent looks in the terminal
