...
Berdaya Agent runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Berdaya Agent
...
1. Berdaya Agent captures a baseline of current diagnostics for the file.
...
…). Berdaya Agent auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Berdaya Agent installs both packages together when you
...
When `install_strategy: auto`, Berdaya Agent installs binaries into
...
shared location — the staging dir is fully Berdaya-owned and is
...
Servers are kept alive for the life of the Berdaya Agent process. There's
