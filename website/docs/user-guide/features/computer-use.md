...
Berdaya Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
window-focus bug for UTM workflows). Berdaya Agent refreshes the binary in two
places so you don't get stuck on a stale release:

- **`hermes update`** — when you update Berdaya Agent itself, if `cua-driver` is
...
Berdaya Agent applies multi-layer guardrails:
...
Screenshots are expensive. Berdaya Agent applies four layers of optimisation:
