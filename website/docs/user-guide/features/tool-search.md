...
**Tool Search** is Berdaya Agent' opt-in progressive-disclosure layer for that
...
:::info Built-in Berdaya Agent tools never defer
The tools that make up Berdaya Agent' core capability set (`terminal`,
...
When the model invokes `tool_call`, Berdaya Agent **unwraps the bridge** and
...
- **No JS sandbox.** Berdaya Agent uses the simpler "structured tools" mode
