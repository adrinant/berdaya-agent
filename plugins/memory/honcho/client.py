...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Berdaya Agent instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Berdaya Agent profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Berdaya Agent profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Berdaya Agent system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Berdaya Agent profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Berdaya Agent session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Berdaya Agent session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Berdaya Agent session_id (new Honcho session each run)
