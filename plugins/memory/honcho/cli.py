...
def cmd_sync(args) -> None:
    ...
    Scans all Berdaya Agent profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Berdaya Agent profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Berdaya Agent persistent cross-session memory.")
...
def _active_profile_name() -> str:
    """Return the active Berdaya Agent profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Berdaya Agent' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Berdaya Agent asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Berdaya Agent + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Berdaya Agent\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Berdaya Agent persistent memory")
    ...
    if user_files:
        ...
        print("  (Berdaya Agent calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Berdaya Agent, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Berdaya Agent:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Berdaya Agent:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Berdaya Agent:   Honcho observes every message and updates representations")
    ...
    print("    Berdaya Agent:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Berdaya Agent Honcho",
    )
