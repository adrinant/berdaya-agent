...
it groups Berdaya Agent-written memories into a dedicated "Berdaya Agent" Space in the
...
When enabled, Berdaya Agent can:
...
Use `{identity}` in the `container_tag` to scope memories per Berdaya Agent profile:
