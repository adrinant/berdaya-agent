...
Optional Berdaya Agent observability plugin that maps Berdaya Agent observer hooks to
...
not replace Berdaya Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Berdaya Agent emit NeMo Relay lifecycle events for
the work Berdaya Agent already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Berdaya Agent can:

- Preserve Berdaya Agent execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
`HERMES_HOME` is the Berdaya Agent profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Berdaya Agent uses the user's default home, usually `~/.hermes`. For isolated smoke
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Berdaya Agent does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Berdaya Agent keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Berdaya Agent routes LLM and tool
...
For the full generic Berdaya Agent middleware contract, see
...
This run starts a parent Berdaya Agent session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Berdaya Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Berdaya Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Berdaya Agent dispatches the read-only tools as one batch, and NeMo Relay
...
- Berdaya Agent sessions map to `agent` scopes.
- Berdaya Agent API request hooks map to `llm` scope start/end events.
- Berdaya Agent tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Berdaya Agent execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Berdaya Agent routes execution
through these boundaries:

```text
Berdaya Agent provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Berdaya Agent provider adapter next_call(...)

Berdaya Agent tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Berdaya Agent tool dispatcher next_call(...)
```
...
middleware for a local Berdaya Agent run. This path requires a NeMo Relay runtime that
...
agent_name = "Berdaya Agent Middleware E2E"
agent_version = "local"
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "Berdaya Agent Middleware E2E",
    "version": "local",
