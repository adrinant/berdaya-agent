"""langfuse — Berdaya Agent plugin for Langfuse observability.

Traces Berdaya Agent conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Berdaya Agent plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Berdaya Agent plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Berdaya Agent turn",
                tags=["hermes", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="Berdaya Agent turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Berdaya Agent branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Berdaya Agent also has a turn-scoped
    ...
    # messages is a list only for legacy Berdaya Agent branches that fired
    # pre_llm_call with API messages directly. Current Berdaya Agent fires
...
def register(ctx) -> None:
    ...
    # Berdaya Agent versions.  pre_api_request / post_api_request fire per API
