...
This plugin connects Berdaya Agent to iMessage (and other Spectrum
...
TypeScript-only, Berdaya Agent runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` are loopback POSTs to the sidecar,
  authenticated with a shared `X-Berdaya-Sidecar-Token`.
...
2. **Find or create** the `Berdaya Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Berdaya Agent channel,
