"""
Photon Spectrum (iMessage) platform adapter for Berdaya Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Berdaya Agent dep
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Berdaya Agent defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Berdaya-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Berdaya-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Berdaya Agent plugin loader at startup."""
