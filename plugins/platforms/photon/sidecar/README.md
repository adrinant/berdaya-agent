...
Small Node helper that bridges Berdaya Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Berdaya Agent is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Berdaya-Sidecar-Token`)
...
The Berdaya Agent plugin's `hermes photon setup` command runs `npm install`
here automatically.
