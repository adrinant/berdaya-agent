// Berdaya Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Berdaya Agent): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Berdaya Agent -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Berdaya-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Berdaya Agent supplied them so a known-good
