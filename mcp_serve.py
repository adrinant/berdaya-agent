"""
Berdaya Agent MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Berdaya-specific extra)
...
class EventBridge:
    ...
    This is the Berdaya Agent equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Berdaya Agent MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Berdaya Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Berdaya Agent MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
