...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Berdaya Agent may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Berdaya Agent gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Berdaya Agent gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Berdaya Agent")
...
def _cmd_migrate(args):
    """Run the OpenClaw → Berdaya Agent migration."""
    ...
    print(
        color(
            "│          ⚕ Berdaya Agent — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    # ── Phase 2b: Pre-apply backup of the Berdaya Agent home ─────────
    ...
    if not no_backup:
        try:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Berdaya Agent home."
            )
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            "│          ⚕ Berdaya Agent — OpenClaw Cleanup                   │",
            Colors.MAGENTA,
        )
    )
    print(
