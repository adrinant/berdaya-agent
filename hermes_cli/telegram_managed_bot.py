...
manual BotFather token copy-paste. Berdaya Agent talks only to the Nous onboarding
...
DEFAULT_MANAGER_BOT = "Berdaya AgentSetupBot"

DEFAULT_BOT_NAME = "Berdaya Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Berdaya Agent Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Berdaya Agent Telegram onboarding service.")
