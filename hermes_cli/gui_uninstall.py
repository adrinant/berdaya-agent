"""
Berdaya Agent Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Berdaya Agent.app`` or ``~/Applications/Berdaya Agent.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Berdaya Agent`` (NSIS per-user)
...
the app name ("Berdaya Agent"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Berdaya Agent``
  - Windows: ``%APPDATA%\\Berdaya Agent``
  - Linux:   ``$XDG_CONFIG_HOME/Berdaya Agent`` (default ``~/.config/Berdaya Agent``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Berdaya Agent"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Berdaya Agent"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "Berdaya Agent"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Berdaya Agent" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Berdaya Agent.app"),
            home / "Applications" / "Berdaya Agent.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Berdaya Agent).
            local_base / "Programs" / "Berdaya Agent",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Berdaya Agent")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Berdaya Agent.desktop",
        ]
    return paths
