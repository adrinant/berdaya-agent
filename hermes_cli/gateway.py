...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            ...
            Berdaya Agent profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Berdaya Agent profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Berdaya Agent Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Berdaya Agent Gateway - Messaging Platform Integration"
...
# Legacy service names from older Berdaya Agent installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Berdaya Agent gateway units.

    Detects unit files installed by older Berdaya Agent versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Berdaya Agent gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Berdaya Agent gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Berdaya Agent gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Berdaya Agent gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Berdaya Agent gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Berdaya Agent gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Berdaya Agent can't create it from this user session."
            )
            if run_as_user:
                print_info(
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Berdaya Agent often sets ``HOME`` to a profile-scoped directory, but
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Berdaya Agent install"
    )
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Berdaya Agent install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Berdaya Agent install")
    else:
        print("⚠ Service definition is stale relative to the current Berdaya Agent install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Berdaya Agent gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Berdaya Agent command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Berdaya Agent Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Berdaya Agent will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Berdaya Agent delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Berdaya Agent agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Berdaya Agent will use (e.g., hermes@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Berdaya Agent connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Berdaya Agent will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Berdaya Agent will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Berdaya Agent will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "Berdaya AgentAgent"')
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy Berdaya Agent gateway unit files from
