...
Diagnoses issues with Berdaya Agent setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Berdaya Agent Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Berdaya Agent' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Berdaya Agent a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Berdaya Agent Persona\n\n"
                "<!-- Edit this file to customize how Berdaya Agent communicates. -->\n\n"
                "You are Berdaya Agent, a helpful AI assistant.\n",
                encoding="utf-8",
            )
