...
on Japanese locales, etc.).  Berdaya Agent's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Berdaya Agent is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Berdaya-managed Git install directories so
    ...
    # install without Berdaya-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Berdaya Agent's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Berdaya Agent's
    ...
    Set this before launching Berdaya Agent (User env var in Windows Settings, or
    export in a PowerShell profile) and Berdaya Agent picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Berdaya-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Berdaya Agent spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Berdaya-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Berdaya Agent venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "berdaya", "berdaya-agent", "venv", "Scripts"),
        os.path.join(local_appdata, "berdaya", "hermes-agent", "venv", "Scripts"),
