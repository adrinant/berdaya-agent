#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Berdaya Agent Skills Hub.
...
# ---------------------------------------------------------------------------
# Berdaya Agent centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Berdaya Agent index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Berdaya Agent index fetch failed: %s", e)
        return _load_stale_index_cache()
