...
the actual `setup.sh` that creates the project workspace, configures Berdaya Agent
...
PyYAML must be installed in the user's Python (it ships with most Berdaya Agent
...
the Berdaya Agent `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
