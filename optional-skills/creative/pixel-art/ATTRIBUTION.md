...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Berdaya Agent uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Berdaya Agent has `image_generate`
...
- Expansion with palettes + video: Berdaya Agent contributors
