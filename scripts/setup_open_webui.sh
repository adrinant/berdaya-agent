...
# Bootstrap Open WebUI against Berdaya Agent's OpenAI-compatible API server.
#
...
#   OPEN_WEBUI_NAME='Johnny Berdaya Agent'
...
#   HERMES_API_MODEL_NAME='Berdaya Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Berdaya Agent WebUI}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Berdaya Agent}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Berdaya Agent
...
main() {
  ...
  log 'Ensuring Berdaya Agent API server is configured...'
  ...
  log 'Restarting Berdaya Agent gateway so API server settings take effect...'
  hermes gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Berdaya Agent API server did not answer on the first check. Trying to start gateway in the background...'
  ...
  log "Berdaya Agent API endpoint: ${HERMES_API_BASE_URL}"
