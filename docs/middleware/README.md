# Berdaya Agent Middleware

Berdaya Agent middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Berdaya Agent'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Berdaya Agent calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Berdaya Agent retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Berdaya Agent guardrails,
...
Berdaya Agent stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Berdaya Agent runs
them as a nested chain in registration order. Middleware failures are fail-open:
Berdaya Agent logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Berdaya Agent applies middleware in this order:
...
For each tool call, Berdaya Agent applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Berdaya Agent pre-execution path against the effective arguments:
...
Return the same response shape Berdaya Agent expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Berdaya Agent treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Berdaya Agent preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Berdaya Agent does not convert downstream
