# Berdaya Agent Observer Hooks

Berdaya Agent observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Berdaya Agent' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Berdaya Agent catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Berdaya Agent only builds
