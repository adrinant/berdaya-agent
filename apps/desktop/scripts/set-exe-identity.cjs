#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Berdaya Agent icon + version metadata onto the
// built Berdaya Agent.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Berdaya Agent.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Berdaya Agent.exe>
//
...
// Stamp the Berdaya Agent icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Berdaya Agent icon + identity stamped')
}
