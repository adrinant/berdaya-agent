 ...
 *
 * Stamps the Berdaya Agent icon + identity onto the packed Windows Berdaya Agent.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Berdaya Agent')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Berdaya Agent'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Berdaya Agent.exe keeps the stock Electron icon`)
  }
}
