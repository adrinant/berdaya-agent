...
const checkBerdaya AgentUpdateSpy = vi.fn()
const updateBerdaya AgentSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/hermes', () => ({
  checkBerdaya AgentUpdate: (...args: unknown[]) => checkBerdaya AgentUpdateSpy(...args),
  updateBerdaya Agent: (...args: unknown[]) => updateBerdaya AgentSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkBerdaya AgentUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkBerdaya AgentUpdateSpy.mockResolvedValue({
      install_method: 'git',
    ...
    expect(checkBerdaya AgentUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkBerdaya AgentUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  beforeEach(() => {
    storage.clear()
    checkBerdaya AgentUpdateSpy.mockReset()
    updateBerdaya AgentSpy.mockReset()
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateBerdaya AgentSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkBerdaya AgentUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'hermes update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateBerdaya AgentSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkBerdaya AgentUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
