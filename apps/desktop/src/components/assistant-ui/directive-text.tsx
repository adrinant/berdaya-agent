...
type Berdaya AgentRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<Berdaya AgentRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as Berdaya AgentRefType] ?? ICON_FALLBACK
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as Berdaya AgentRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: Berdaya AgentRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Berdaya Agent directives (`@file:...`, `@image:...`) as
