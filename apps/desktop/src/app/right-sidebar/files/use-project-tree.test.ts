...
import type { Berdaya AgentReadDirResult } from '@/global'
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): Berdaya AgentReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: Berdaya AgentReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<Berdaya AgentReadDirResult>(resolve => {
