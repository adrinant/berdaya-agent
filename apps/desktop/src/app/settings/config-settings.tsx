...
import {
  getElevenLabsVoices,
  getBerdaya AgentConfigDefaults,
  getBerdaya AgentConfigRecord,
  getBerdaya AgentConfigSchema,
  saveBerdaya AgentConfig
} from '@/hermes'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, Berdaya AgentConfigRecord } from '@/types/hermes'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<Berdaya AgentConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<Berdaya AgentConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getBerdaya AgentConfigRecord(), getBerdaya AgentConfigDefaults(), getBerdaya AgentConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await saveBerdaya AgentConfig(config)
  ...
  const updateConfig = (next: Berdaya AgentConfigRecord) => {
