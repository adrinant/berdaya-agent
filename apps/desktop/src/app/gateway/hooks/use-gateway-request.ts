...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<Berdaya AgentGateway | null>(null)
  ...
  const reconnectingRef = useRef<Promise<Berdaya AgentGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as Berdaya AgentGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Berdaya Agent gateway unavailable')
      }
