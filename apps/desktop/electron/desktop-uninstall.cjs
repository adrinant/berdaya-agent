 ...
 *
 *   macOS:   …/Berdaya Agent.app/Contents/MacOS/Berdaya Agent  → …/Berdaya Agent.app
 *   Windows: …\Berdaya Agent\Berdaya Agent.exe                 → …\Berdaya Agent  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Berdaya Agent.app/Contents/MacOS/Berdaya Agent → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Berdaya Agent.app
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  // a problem, but Berdaya Agent install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
