...
import { launchBerdaya AgentDesktop } from '../store'
 ...
 * Berdaya Agent.exe doesn't exist). We catch the Tauri error and surface it
 * inline rather than silently doing nothing — the previous version
 * had `onClick={() => void launchBerdaya AgentDesktop()}` which swallowed
...
export default function Success() {
  ...
  async function handleLaunch() {
    ...
    setLaunching(true)
    try {
      await launchBerdaya AgentDesktop()
  ...
  return (
    <div className="hermes-fade-in flex h-full flex-col items-center justify-center gap-8 px-12 py-10">
      <div className="w-full max-w-2xl min-w-0 text-center">
        ...
        >
          <span>
            <span>Berdaya Agent is ready</span>
          </span>
          <span aria-hidden="true">Berdaya Agent is ready</span>
        </p>
      ...
      >
        <Rocket size={18} />
        {launching ? 'Launching…' : 'Launch Berdaya Agent'}
      </Button>
