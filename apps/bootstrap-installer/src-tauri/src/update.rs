...
//!
//! Driven when the installer is launched as `Berdaya-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Berdaya Agent desktop process to fully exit (so the venv
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Berdaya Agent"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let hermes = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the hermes CLI under {}. Is Berdaya Agent installed? \
    ...
    // and the guard would only produce a false "Berdaya Agent is still running" stop.
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Berdaya Agent" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Berdaya Agent is still running. Close all Berdaya Agent windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Berdaya Agent manually."),
            );
        }
    } else if let Err(err) =
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Berdaya Agent to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            emit_log(
                ...
                Some("update"),
                LogStream::Stdout,
                "[update] Berdaya Agent still holding the venv shim; force-killing stragglers…",
            );
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Berdaya Agent.app was found under {}",
...
mod tests {
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Berdaya Agent.app"]),
            Some(PathBuf::from("/Applications/Berdaya Agent.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Berdaya Agent.app");
        let tmp = base.join("Berdaya Agent.app.hermes-update-new");
        let old = base.join("Berdaya Agent.app.hermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Berdaya Agent.app");
        let tmp = base.join("Berdaya Agent.app.hermes-update-new"); // intentionally absent
        let old = base.join("Berdaya Agent.app.hermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Berdaya Agent.app");
        let tmp = base.join("Berdaya Agent.app.hermes-update-new"); // absent
        let old = base.join("Berdaya Agent.app.hermes-update-old");
        write_marker(&target, "OLD");
