//! Berdaya Agent Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `Berdaya-Setup.exe --update`).
///
...
pub fn run() {
    ...
    // Berdaya Agent is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Berdaya Agent installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Berdaya Agent is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Berdaya Agent" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Berdaya Agent" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::agent_install_dir();
        ...
        .expect("error while running Berdaya Agent Setup");
}
