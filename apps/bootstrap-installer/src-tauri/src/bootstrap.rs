...
/// Spawn the locally-built Berdaya Agent desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Berdaya Agent desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching Berdaya Agent desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Berdaya Agent
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Berdaya Agent.exe"),
            ("win-arm64-unpacked", "Berdaya Agent.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Berdaya Agent.app/Contents/MacOS", "Berdaya Agent"),
            ("mac-arm64/Berdaya Agent.app/Contents/MacOS", "Berdaya Agent"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Berdaya Agent.app/Contents/MacOS/Berdaya Agent -> .../Berdaya Agent.app
...
/// path so a bare re-open just opens Berdaya Agent instead of re-running setup.
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Berdaya Agent desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // 4. Resolve install_root. install.ps1 clones the repo into
    // $BerdayaHome\berdaya-agent (legacy: hermes-agent). Mirrors hermes_constants.
    let hermes_home = args
        .hermes_home
        .clone()
        .unwrap_or_else(|| crate::paths::hermes_home().to_string_lossy().into_owned());
    let install_root = crate::paths::agent_install_dir();
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            let macos_dir = release
                .join("mac-arm64")
                .join("Berdaya Agent.app")
                .join("Contents")
                .join("MacOS");
            std::fs::create_dir_all(&macos_dir).unwrap();
            std::fs::write(macos_dir.join("Berdaya Agent"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Berdaya Agent.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Berdaya Agent.exe");
    ...
    // what the updater ditto's over /Applications/Berdaya Agent.app). A regression in
