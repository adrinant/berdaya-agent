...
Connects Berdaya Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Berdaya Agent adapter for Weixin personal accounts."""
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Berdaya Agent regardless of this "
                "policy. If group delivery doesn't work, the limitation is on the iLink side, "
                "not in Berdaya Agent.",
                self.name,
                self._group_policy,
            )
        return True
