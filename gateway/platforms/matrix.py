...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Berdaya Agent command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Berdaya Agent accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Berdaya Agent ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Berdaya Agent
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Berdaya Agent" become "Agent".
        """
        if not body:
            return ""
