...
# Audio file extensions Berdaya Agent recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Berdaya Agent' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Berdaya-created Telegram private-chat
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; Berdaya AgentAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Berdaya Agent profile and shared Berdaya Agent root both contain control
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Berdaya Agent
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Berdaya-managed cache, under an operator-allowlisted root
...
class BasePlatformAdapter(ABC):
    ...
    # Berdaya Agent commands.  Default "/" (most platforms deliver "/approve" etc.
