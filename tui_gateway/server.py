...
class _SlashWorker:
    """Persistent Berdaya AgentCLI subprocess for slash commands."""
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Berdaya Agent): with no explicit pin, collapse to the
...
def _load_fallback_model():
    ...
    stays in parity with ``Berdaya AgentCLI.__init__`` and ``gateway/run.py``:
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Berdaya Agent profile home
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Berdaya Agent Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Berdaya Agent/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Berdaya Agent Desktop. If it serves Berdaya Agent/Desktop UI or another unrelated app, stop that process and report failure.",
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Berdaya Agent provider is configured.",
                },
            )
