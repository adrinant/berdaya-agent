"""Persistent slash-command worker — one Berdaya AgentCLI per TUI session.
...
def _run(cli: Berdaya AgentCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) Berdaya AgentCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = Berdaya AgentCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
