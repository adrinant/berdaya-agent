# Berdaya Agent - Development Guide

Instructions for AI coding assistants and developers working on the hermes-agent codebase.

**Never give up on the right solution.**

## What Berdaya Agent Is

Berdaya Agent is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Berdaya Agent ships a **lot** — most merges are bug fixes to
...
├── cli.py                # Berdaya AgentCLI class — interactive CLI orchestrator (~11k LOC)
...
Applies to TypeScript across Berdaya Agent: desktop, TUI, website, and future TS packages.
...
- `process_command()` is a method on `Berdaya AgentCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
...
```
2. Add handler in `Berdaya AgentCLI.process_command()` in `cli.py`:
```python
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Berdaya Agent built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Berdaya Agent core.
...
core Berdaya Agent tool that should ship in the base system.
...
- `default` — Classic Berdaya Agent gold/kawaii (the current look)
...
Berdaya Agent has two plugin surfaces. Both live under `plugins/` in the repo so
...
2. **Tools referenced in SKILL.md prose must be native Berdaya Agent tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Berdaya Agent" is the secondary collaborator. If the
   contributor's commit shows "Berdaya Agent" as author (because they
   used Berdaya Agent to draft the skill), replace it with their actual name
...
Berdaya-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Berdaya Agent supports **profiles** — multiple fully isolated instances, each with its own
