...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Berdaya Agent layout and point HERMES_HOME at
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        ...
        assert "Active Berdaya Agent profile" in src
        assert "cross_profile=True" in src
        assert "~/.hermes/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Berdaya Agent profile: default" in src
        assert "Active Berdaya Agent profile: {active_profile}" in src
