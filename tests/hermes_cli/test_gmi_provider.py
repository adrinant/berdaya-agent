...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a Berdaya AgentAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("Berdaya AgentAgent/")

    def test_gmi_profile_declares_hermes_user_agent(self):
        """The GMI plugin sets a Berdaya AgentAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("Berdaya AgentAgent/"), (
            f"expected GMI profile User-Agent to start with 'Berdaya AgentAgent/', got {ua!r}"
        )
