...
class TestMigrate:
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        into Berdaya Agent for browser/web/delegate_task/vision/memory tools.
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Berdaya Agent appends
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Berdaya Agent' managed block, re-running
...
class TestBerdayaHomeLeakGuard:
