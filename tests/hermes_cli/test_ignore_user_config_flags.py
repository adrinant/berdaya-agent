...
files. In Berdaya Agent the equivalent isolation is:
...
argparse → cmd_chat → cli.main() → Berdaya AgentCLI → AIAgent call chain.
"""
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``Berdaya AgentCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HERMES_IGNORE_RULES=1 flips Berdaya AgentCLI.ignore_rules True."""
        monkeypatch.setenv("HERMES_IGNORE_RULES", "1")

        # Import Berdaya AgentCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of Berdaya AgentCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.Berdaya AgentCLI)
        # Replicate the exact logic from cli.py Berdaya AgentCLI.__init__:
