...
Verifies that ``Berdaya AgentCLI._handle_update_command`` correctly:
...
def _make_self(modal_response):
    ...
    so we don't need a full ``Berdaya AgentCLI`` construction.
...
def _call(self_):
    ...
    return Berdaya AgentCLI._handle_update_command(self_)
