...
def _make_real_cli(**kwargs):
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return cli_mod.Berdaya AgentCLI(**kwargs)
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "Berdaya AgentCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
...
def test_main_raises_for_unknown_preloaded_skill(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "Berdaya AgentCLI", lambda **kwargs: _DummyCLI(**kwargs))
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
