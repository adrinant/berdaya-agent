...
def _make_cli():
    """Bare-metal Berdaya AgentCLI for snapshot/build tests (no __init__ side effects)."""
...
def test_snapshot_safe_when_background_tasks_attr_missing():
    """Older Berdaya AgentCLI instances (tests with __new__, etc.) may lack the attr."""
    cli_obj = Berdaya AgentCLI.__new__(Berdaya AgentCLI)
    cli_obj.model = "x"
    cli_obj.agent = None
    cli_obj.session_start = datetime.now()
