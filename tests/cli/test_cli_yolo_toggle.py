...
Pre-fix bug (issue #33925): ``cli.Berdaya AgentCLI._toggle_yolo`` mutated only
...
read (``session_id``). This avoids the heavy ``Berdaya AgentCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``Berdaya AgentCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``Berdaya AgentCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``Berdaya AgentCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            Berdaya AgentCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            Berdaya AgentCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            Berdaya AgentCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
    ...
    def test_two_independent_sessions_are_isolated(self):
        ...
        try:
            with patch("cli._cprint"):
                Berdaya AgentCLI._toggle_yolo(cli_a)
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert Berdaya AgentCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            Berdaya AgentCLI._toggle_yolo(stand_in)

        assert Berdaya AgentCLI._is_session_yolo_active(stand_in) is True
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                Berdaya AgentCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``Berdaya AgentCLI.__new__(Berdaya AgentCLI)`` to skip ``__init__``). It must
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert Berdaya AgentCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            Berdaya AgentCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            Berdaya AgentCLI._transfer_session_yolo(stand_in, "same-id", "same-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        Berdaya AgentCLI._transfer_session_yolo(stand_in, "", "new")
        Berdaya AgentCLI._transfer_session_yolo(stand_in, "old", "")
