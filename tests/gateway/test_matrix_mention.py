...
class TestIsBotMentioned:
    ...
    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@hermes:example.org">Berdaya Agent</a> help'
        assert self.adapter._is_bot_mentioned("Berdaya Agent help", html)
...
class TestStripMention:
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "Berdaya Agent" should not be mutated.
        result = self.adapter._strip_mention("Berdaya Agent")
        assert result == "Berdaya Agent"
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@hermes:example.org">Berdaya Agent</a> help'
    event = _make_event("Berdaya Agent help", formatted_body=formatted)
