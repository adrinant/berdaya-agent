...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_hermes_variants(self, monkeypatch):
        ...
        assert adapter._message_matches_mention_patterns("Berdaya Agent, summarize this")
        assert adapter._message_matches_mention_patterns("@Berdaya Agent agent help")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Berdaya Agent what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Berdaya Agent, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Berdaya Agent agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Berdaya Agent about this") == "please ask Berdaya Agent about this"
...
class TestBlueBubblesMentionGating:
    ...
    async def test_group_message_with_default_mention_is_dispatched_cleaned(self, monkeypatch):
        ...
        response = await adapter._handle_webhook(_FakeBlueBubblesRequest({
            ...
            "data": {
                "guid": "msg-2",
                "text": "Berdaya Agent, summarize this",
                "handle": {"address": "+15555550100"},
                "isFromMe": False,
                "isGroup": True,
                "chats": [{"guid": "iMessage;+;group-chat"}],
