    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", "X-Berdaya-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-Berdaya-Session-Id"] == session_id
    assert resp.headers["X-Berdaya-Session-Key"] == "client-42"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-Berdaya-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-Berdaya-Session-Key requires API key" in data["error"]["message"]
