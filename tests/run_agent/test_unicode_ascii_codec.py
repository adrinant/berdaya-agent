...
class TestSanitizeStructureNonAscii:
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                "X-Title": "Berdaya Agent │ Agent",
                "User-Agent": "Berdaya Agent/1.0 🤖",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "Berdaya Agent  Agent"
        assert payload["default_headers"]["User-Agent"] == "Berdaya Agent/1.0 "
