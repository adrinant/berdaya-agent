...
"""
    ...
    assert "Tyler Williams > Active Projects: Berdaya Agent" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Berdaya Agent``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Berdaya Agent prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Berdaya Agent to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Berdaya Agent config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Berdaya Agent uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (Berdaya Agent home) when rebranding
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Berdaya Agent remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "hermes prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Berdaya Agent was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Berdaya Agent said to ask Berdaya Agent about Berdaya Agent settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.hermes`` (the real Berdaya Agent home), not the broken
    ``.Berdaya Agent``.
    ...
    rewritten to ``~/.Berdaya Agent/config.yaml`` — a path that doesn't exist —
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    assert "Berdaya Agent" in result
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Berdaya Agent" in result
