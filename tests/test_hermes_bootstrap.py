...
The bootstrap module is imported at the top of every Berdaya Agent entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every Berdaya Agent entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Berdaya Agent CLI
...
class TestEntryPointsImportBootstrap:
    """Every Berdaya Agent entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Berdaya Agent as a process.  Each one must
