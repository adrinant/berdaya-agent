...
def test_root_session_no_compression(db):
    ...
    assert prov["currentBerdaya AgentSessionId"] == "root1"
    assert prov["rootBerdaya AgentSessionId"] == "root1"
    assert prov["parentBerdaya AgentSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    assert prov["parentBerdaya AgentSessionId"] == "old"
    assert prov["rootBerdaya AgentSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousBerdaya AgentSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootBerdaya AgentSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootBerdaya AgentSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    assert prov["previousBerdaya AgentSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert meta["hermes"]["sessionProvenance"]["currentBerdaya AgentSessionId"] == "root1"
