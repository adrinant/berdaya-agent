"""Tests for acp_adapter.server — Berdaya AgentACPAgent ACP server."""
...
from acp_adapter.server import Berdaya AgentACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """Berdaya AgentACPAgent backed by a mock session manager."""
    return Berdaya AgentACPAgent(session_manager=mock_manager)
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Berdaya Agent' interactive model/provider setup in a terminal. "
                    "Use this when Berdaya Agent has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Berdaya Agent provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = Berdaya AgentACPAgent(session_manager=manager)
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "Berdaya AgentACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("Berdaya AgentACPAgent")
