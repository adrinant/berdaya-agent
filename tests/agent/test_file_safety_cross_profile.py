"""Tests for the cross-Berdaya Agent-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Berdaya Agent profile's
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Berdaya Agent root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    """Build a fake Berdaya Agent layout:
...
class TestClassifyCrossProfileTarget:
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Berdaya Agent root
