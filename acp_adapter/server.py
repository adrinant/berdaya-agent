"""ACP agent server — exposes Berdaya Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Berdaya Agent running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Berdaya Agent cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Berdaya Agent/OpenAI-compatible user content payload."""
...
class Berdaya AgentACPAgent(acp.Agent):
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Berdaya Agent version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Berdaya Agent version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Berdaya Agent maps edit approval
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Berdaya Agent estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Berdaya Agent changes it.

        When the internal Berdaya Agent head rotated (e.g. compression-driven session
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Berdaya Agent' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Berdaya Agent setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Berdaya Agent
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Berdaya Agent on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Berdaya Agent' local kawaii waiting/status
        ...
        try:
            # Snapshot the internal Berdaya Agent DB session id before the turn so we
        ...
        # Berdaya Agent' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Berdaya Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Berdaya Agent has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
