"""ACP auth helpers — detect and advertise Berdaya Agent authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Berdaya Agent runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Berdaya Agent can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Berdaya Agent.
    ...
    not have Berdaya Agent provider credentials configured yet, so Berdaya Agent always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Berdaya Agent using the currently configured "
