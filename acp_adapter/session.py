"""ACP session manager — maps ACP sessions to Berdaya Agent AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Berdaya Agent itself is running in WSL.
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Berdaya Agent from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Berdaya Agent agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Berdaya Agent AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Berdaya Agent runtime provider configuration.
